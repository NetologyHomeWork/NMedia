...
import ru.netology.nmedia.database.mapper.DbObjectMapper
import ru.netology.nmedia.database.repository.PostDaoRepository
import ru.netology.nmedia.model.Post
import ru.netology.nmedia.utils.parsingUrlLink

class PostRepositoryImpl(
    private val daoRepository: PostDaoRepository
) : PostRepository {

    private val mapper = DbObjectMapper()

    private val data = Transformations.map(daoRepository.allPostEntity) {
        mapper.mapListEntityToListDTO(it)
    }

    init {
        /*daoRepository.save(mapper.mapDTOtoEntity(
            Post(
                id = 0,
    ...
    override fun like(post: Post) {
        ...
        daoRepository.like(mapper.mapDTOtoEntity(newPost))
    }
    ...
    override fun share(post: Post): Intent {
        ...
        daoRepository.share(mapper.mapDTOtoEntity(newPost))
    ...
    override fun savePost(post: Post) {
        daoRepository.save(mapper.mapDTOtoEntity(post))
    }
    ...
    override fun findPostById(postId: Long): Post? {
        val entity = daoRepository.findPostById(postId)
        return entity?.let { mapper.mapEntityToDTO(it) }
    }
