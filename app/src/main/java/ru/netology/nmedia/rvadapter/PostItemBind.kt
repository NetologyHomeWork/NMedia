...
import ru.netology.nmedia.fragments.PostDetailFragmentDirections
...
fun bindPostItemLayout(
    ...
    bindingItem(binding, post)
    viewModelListenerAction(context, binding, post, viewModel, navController)
}
...
private fun viewModelListenerAction(
    ...
    with(binding) {
        ...
        if (videoView.visibility == View.VISIBLE) {
            videoView.setOnClickListener {
                viewModel.launchYoutubeVideo(post)
            }
        }
    }
}